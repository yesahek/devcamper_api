...
exports.getBootcamps = asyncHandler(async (req, res, next) => {
    ...
    // // Loop over reomveFields and delete them from reqQuery
    ...
    // queryStr = queryStr.replace(/\b(gt|gte|lt|lte|in)\b/g, mathc => `$${match}`);
...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if(publishedBootcamp && req.user.role !== 'admin'){
        return next(
            new ErrorResponse(`The user with Id ${req.user.id} has already published a bootcamp`,
                400
            )
        );
    }    
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
 ...
 file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async err =>{
    ...
    if(err){
        return next(
            new ErrorResponse(`Unkown error occured! Try agin letter`, 500)
        );
    }
